...
const respuestaModel = require('../models/respuestaModel'); // Ruta para actualizar la respuesta de un ticket

// Ruta para actualizar la respuesta de un ticket
router.put('/', async (req, res) => { // Cambiado a '/respuesta' para especificar la ruta
    const { ticket_id, solucion } = req.body; // Obtener ticket_id y solucion del cuerpo de la solicitud
    ...
    try {
        ...
        const actualizado = await respuestaModel.actualizarRespuesta(ticket_id, solucion);
        if (actualizado) {
